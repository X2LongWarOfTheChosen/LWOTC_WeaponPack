...
static function bool ShouldRemoveAreaSuppression(XComGameState_Unit SourceUnit, optional XComGameState NewGameState, optional bool bBeforeAmmoReduction = false)
{
	...
	if (SourceUnit != none)
	{
		...
		if (WeaponState != none)
		{
			...
			{
				if (WeaponState.Ammo <= (2 * class'X2Ability_LMGAbilities'.default.AREA_SUPPRESSION_SHOT_AMMO_COST) - 1)
					bShouldRemove = true;
			}
			else if (WeaponState.Ammo < (2 * class'X2Ability_LMGAbilities'.default.AREA_SUPPRESSION_SHOT_AMMO_COST) - 1)
			{
				bShouldRemove = true;
			}
		}
...
DefaultProperties
{
	EffectName="LWotC_CannonAreaSuppression"
}